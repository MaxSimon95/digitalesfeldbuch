    ...
    <form>
      ...
      <ion-item>Befund (SE)
        <p v-if="availableStructures.length === 0">
          <ion-icon name="information-circle"></ion-icon>
          Es wurden bisher noch keine Befunde hinterlegt.
        </p>
      ...
      <ion-item>Material
        <p v-if="availableMaterials.length === 0">
          <ion-icon name="information-circle"></ion-icon>
          Es wurden bisher noch keine Materialien hinterlegt.
        </p>
        <ion-select v-else interface="popover" placeholder="Fundmaterial bestimmen"
                    v-on:ionChange="affiliatedMaterial=$event.target.value">

          <ion-select-option v-for="item in lastUsedMaterials" v-bind:key="'lum'+item" lines="inset"
                             v-bind:value="item">
            <ion-text>
              Zuletzt: {{item}}
            </ion-text>
      ...
      <ion-item>Typ
        <p v-if="availableTypes.length === 0">
          <ion-icon name="information-circle"></ion-icon>
          Es wurden bisher noch keine Typen hinterlegt.
        </p>
        <ion-select v-else placeholder="Fundart bestimmen" interface="popover"
                    v-on:ionChange="type=$event.target.value">
          ...
          <ion-select-option v-for="item in availableTypes" v-bind:key="item" lines="inset" v-bind:value="item"
                             selected="false">
            <ion-text>
              {{item}}
            </ion-text>
      ...
      <ion-item>
        ...
        <ion-input v-on:ionInput="tachymeterid=$event.target.value"
                   placeholder="Geben sie hier die Tachymeter ID für die spätere Verortung ein"></ion-input>
      </ion-item>

      <ion-item>
        <ion-label position="stacked">Vorläufige Datierung</ion-label>
        <ion-input v-on:ionInput="prelimdate=$event.target.value"
                   placeholder="Geben Sie hier eine vorläufige Datierung des Fundes ein"></ion-input>
      </ion-item>

      <ion-item>
        <ion-label position="stacked">Anmerkung</ion-label>
        <ion-textarea v-on:ionInput="description=$event.target.value" rows="4"
                      placeholder="Geben Sie hier weitere Anmerkungen zu dem Fund an"></ion-textarea>
      </ion-item>
  ...
  export default {
    ...
    methods: {
      ...
      getTypes: function () {
        ...
        //console.log(context.lastUsedTypes)
      },
      ...
      logForm: function () {
        ...
        console.log("aff mat")
        console.log(this.affiliatedMaterial)
        // update last used materials and types
        if (
          (
            this.affiliatedMaterial !== ''
          )
          &&
